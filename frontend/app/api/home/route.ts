...
export async function GET(req: NextRequest) {
    try {
        ...
        // Fetch Carousel Novels (up to 12 active novels with at least one published chapter)
        const carouselActiveNovels = await prisma.novel.findMany({
            where: {
                is_active: true,
        ...
        const formattedCarouselNovels = formatNovels(carouselActiveNovels);
        ...
        if (user) {
            ...
            return NextResponse.json({
                ...
                newlyUpdatedHistory,
                carouselNovels: formattedCarouselNovels
            });
        }

        // For Guest (Logged Out) Users: Return the standard landing public feeds
        // Fetch Top Viewed — only novels with at least one published chapter
        ...
        // Fetch Newly Updated — only novels with at least one published chapter
        ...
        // Fetch Top Rated — only novels with at least one published chapter
